...
#define Sahara_LS_GOD 0

#define Sahara_LS_RLS 1
...
struct DeferredRouteOutputTag : public Tag
{
  ...
  {
    static TypeId tid = TypeId ("ns3::Sahara::DeferredRouteOutputTag").SetParent<Tag> ();
    return tid;
  }
...
#define Sahara_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-Sahara_MAXJITTER)-Sahara_MAXJITTER] used to jitter HELLO packet transmission.

#define JITTER (Seconds (x->GetValue (-Sahara_MAXJITTER, Sahara_MAXJITTER))) 
#define FIRST_JITTER (Seconds (x->GetValue (0, Sahara_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4



NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for Sahara control traffic, not defined by IANA yet
const uint32_t RoutingProtocol::Sahara_PORT = 666;
...
{
  static TypeId tid = TypeId ("ns3::Sahara::RoutingProtocol")
    ...
    .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                   EnumValue (Sahara_LS_GOD),
                   MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                   MakeEnumChecker (Sahara_LS_GOD, "GOD",
                                    Sahara_LS_RLS, "RLS"))
...
NS_LOG_FUNCTION (this << p->GetUid () << header.GetDestination () << idev->GetAddress ());
  if (m_socketAddresses.empty ())
    {
      NS_LOG_LOGIC ("No Sahara interfaces");
      return false;
    }
  NS_ASSERT (m_ipv4 != 0);
  NS_ASSERT (p != 0);
  ...
  if (m_ipv4->IsDestinationAddress (dst, iif))
    {
      ...
      TypeHeader tHeader (SaharaTYPE_POS);
      packet->RemoveHeader (tHeader);
      if (!tHeader.IsValid ())
        {
          NS_LOG_DEBUG ("Sahara message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
          return false;
        }
      
      if (tHeader.Get () == SaharaTYPE_POS)
        {
          PositionHeader phdr;
        ...
        while(m_queue.Dequeue (dst, queueEntry))
          {
            ...
            if (!tHeader.IsValid ())
              {
                NS_LOG_DEBUG ("Sahara message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
                return false;     // drop
              }
            if (tHeader.Get () == SaharaTYPE_POS)
              {
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
  ...
  if (l3->GetNAddresses (interface) > 1)
    {
      NS_LOG_WARN ("Sahara does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvSahara, this));
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), Sahara_PORT));
...
void
RoutingProtocol::RecvSahara (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  Address sourceAddress;
  Ptr<Packet> packet = socket->RecvFrom (sourceAddress);

  TypeHeader tHeader (SaharaTYPE_HELLO);
  packet->RemoveHeader (tHeader);
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("Sahara message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
...
void RoutingProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses ((interface) == 1))
    ...
    {
      NS_LOG_LOGIC ("Sahara does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in Sahara operation");
    }
}
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      socket->SendTo (packet, 0, InetSocketAddress (destination, Sahara_PORT));
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif)
{
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid Sahara source address not found");
